...
/*   Updated: 2023/02/11 05:00:57 by oessamdi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	init_directions(t_mlx *mlx)
{
	...
	mlx->mp->d = malloc(sizeof(t_img));
}

void	init_imgs(t_mlx *mlx, t_game *game)
{
	...
	mlx->mp->d->path = game->door;
	mlx->mp->d->img_ptr = mlx_xpm_file_to_image(mlx->mlx_ptr,
			mlx->mp->d->path, &mlx->mp->d->width, &mlx->mp->d->height);
}
...
void	get_textures(t_mlx *mlx, t_game *game)
{
	...
	if (!mlx->mp->no->img_ptr || !mlx->mp->so->img_ptr || !mlx->mp->we->img_ptr
		|| !mlx->mp->ea->img_ptr || !mlx->mp->d->img_ptr)
		error_path();
	...
	mlx->mp->d->data = mlx_get_data_addr(mlx->mp->d->img_ptr,
			&mlx->mp->d->bpp, &mlx->mp->d->size_l, &mlx->mp->d->endian);
}
